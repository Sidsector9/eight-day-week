<?php
/**
 * Handles the user roles functionality
 *
...
function post_type_updated_labels( $messages ) {
	...
	$messages[ EDW_PRINT_ISSUE_CPT ] = array(
		...
		/* translators: %s is replaced with the revision title */
 ...
 * @param int   $maxdepth The max number of recursions to perform.
 ...
 * @return array
 */
function array_keys_recursive( $ary, $maxdepth = INF, $depth = 0, $array_keys = array() ) {
	if ( $depth < $maxdepth ) {
		++$depth;
		$keys = array_keys( $ary );
		foreach ( $keys as $key ) {
			if ( is_array( $ary[ $key ] ) ) {
				$array_keys[ $key ] = array_keys_recursive( $ary[ $key ], $maxdepth, $depth );
			}
		}
	}
 ...
 *
 * @param mixed  $orig The value to return, either a single metadata value or an array of values depending on the value of $single.
