...
function setup() {
	...
	// Load publish date use -1 priority to ensure its loaded before 3rd party plugins.
...
class AL_Table extends \WP_Posts_List_Table {
	 ...
	 *
	 * @param string $which The area of the table nav.
	 */
	...
	public function get_data( $item ) {
		...
		foreach ( (array) $this->get_columns() as $key => $title ) {
			...
			// Using object buffering because some WP_Posts_List_Table methods
 ...
 * @throws \Exception Errors when title isn't valid/empty or no articles found.
 */
...
function save_section_articles( $post_id, $post, $update ) {
	...
	foreach ( $article_ids_sets as $section_id => $article_ids ) {
		// Validate section.
		if ( ! $section_id || ! get_post( $section_id ) === $section ) {
			continue;
		}
