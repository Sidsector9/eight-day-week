<?php
/**
 * Handles the core plugin functionality
 *
 ...
 *
 * @param false|string $query_arg Key to check for the nonce in $_REQUEST
 *                                (since 2.5). If false, $_REQUEST values will
 *                                be evaluated for '_ajax_nonce', and '_wpnonce'
 *                                (in that order).
 * @param bool         $stop Whether to stop early when the nonce cannot be
 *                           verified.
 */
function check_ajax_referer( $query_arg = false, $stop = false ) {
	\check_ajax_referer( EDW_AJAX_NONCE_SLUG, $query_arg, $stop );
}

/**
 * Checks the ajax referer based on a constant nonce slug and ensures the
 * request comes from an elevated (print editor) user.
 *
 * @param string       $action The action nonce.
 * @param false|string $query_arg Key to check for the nonce in $_REQUEST
 *                                (since 2.5). If false, $_REQUEST values will
 *                                be evaluated for '_ajax_nonce', and '_wpnonce'
 *                                (in that order).
 ...
 *                           verified.
 */
function check_elevated_ajax_referer( $action = false, $query_arg = false, $stop = false ) {
	check_ajax_referer( $query_arg, $stop );

	// If the user didn't pass in an $action, check $_POST. One can pass an
	// empty string to use no cap/action.
	$action = false === $action && isset( $_POST['action'] )
